...
def create_metrics_config(eval_type_factory: GroupFactory) -> List[Dict[str, Any]]:
    ...
    for category_name in eval_type_factory.get_available_names():
        ...
        for metric_name in category_factory.get_available_names():
            # TODO add metric prefilters for testing
...
def test_parse_missing_evaluation(eval_type: str, parse_event_dispatcher: EventDispatcher) -> None:
    ...
    parsed_metrics = parse_evaluation_config({}, eval_type_factory, parse_event_dispatcher)
...
@pytest.mark.parametrize('eval_type', VALID_TYPES)
def test_parse_evaluation_config(eval_type: str, parse_event_dispatcher: EventDispatcher) -> None:
