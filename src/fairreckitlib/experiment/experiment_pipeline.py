...
class ExperimentPipeline:
    ...
    def run(self,
        ...
        Raises:
            ...
            RuntimeError: when no data transitions are generated by the data or model pipelines.
        """
        if os.path.isdir(output_dir):
            raise IOError('Experiment pipeline output directory already exists')
        ...
        if len(data_result) == 0:
            self.event_dispatcher.dispatch(ErrorEventArgs(
                ON_FAILURE_ERROR,
                'Failure: to generate experiment data transitions'
            ))
            raise RuntimeError('Experiment failed to generate data transitions')
        ...
        for data_transition in data_result:
            ...
            model_dirs = run_model_pipelines(
                ModelPipelineConfig(
                    ...
                    data_transition,
                    model_factory.get_factory(experiment_config.get_type()),
            ...
            if len(model_dirs) == 0:
                self.event_dispatcher.dispatch(ErrorEventArgs(
                    ON_FAILURE_ERROR,
                    'Failure: to compute experiment model ratings for data transition'
                ))
                continue

            computed_models += len(model_dirs)
            ...
            if len(experiment_config.evaluation) > 0:
                ...
                run_evaluation_pipelines(
                    EvaluationPipelineConfig(
                        ...
                        evaluation_factory.get_factory(experiment_config.get_type()),
        ...
        if computed_models == 0:
            self.event_dispatcher.dispatch(ErrorEventArgs(
                ON_FAILURE_ERROR,
                'Failure: to compute any experiment model ratings'
            ))
            raise RuntimeError('Experiment failed to compute any models')
